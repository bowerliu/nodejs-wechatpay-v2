...
/*  收银员使用扫码设备读取微信用户付款码以后，二维码或条码信息会传送至商户收银台，由商户收银台或者商户后台调用该接口发起支付。
	...
	pay_bank(opt,complete){
		let url = "https://api.mch.weixin.qq.com/mmpaysptrans/pay_bank";		
		let {enc_bank_no,enc_true_name,bank_code,money,desc} = opt;
		let nonce_str = randomStr();
		let params ={
			mch_appid:this.appid,
			mchid:this.mch_id,
	...
	refund(opt,complete){
		...
		let {total_fee,refund_fee,refund_desc,out_trade_no,notify_url,transaction_id} = opt;
		let that = this;
		let nonce_str = randomStr();
	...
	batchquerycomment(opt,complete){ //拉取订单评价数据
		let url = `https://api.mch.weixin.qq.com/billcommentsp/batchquerycomment`;
		let {offset,begin_time,end_time,limit} = opt;
		let that = this;
		let nonce_str = randomStr();
		let params ={
			appid:that.appid,
